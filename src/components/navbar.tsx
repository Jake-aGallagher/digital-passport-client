...
const Navbar = () => {
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 h-12 md:text-xl text-white flex flex-row justify-start md:justify-center items-center gap-4 md:gap-12">
            ...
            {loggedIn ? (
                <button onClick={() => setLoggedIn(false)} className="absolute right-0 md:right-12 rounded-lg h-10 w-20 flex flex-col justify-center items-center hover:text-blue-300 transition-all">Logout</button>
            ) : (
                <>
